...
public class Percolation {
    ...
    public void open(int row, int col) {
        ...
        if (row - 1 > 0 && isOpen(row - 1, col))
            id.union(convertThePositionAsIdIndex(row, col), convertThePositionAsIdIndex(row - 1, col));
        if (row + 1 <= N && isOpen(row + 1, col))
            id.union(convertThePositionAsIdIndex(row, col), convertThePositionAsIdIndex(row + 1, col));
        if (col - 1 > 0 && isOpen(row, col - 1))
            id.union(convertThePositionAsIdIndex(row, col), convertThePositionAsIdIndex(row, col - 1));
    ...
    public boolean isFull(int row, int col) {
        if (id.connected(topIndex, convertThePositionAsIdIndex(row, col)) && isOpen(row, col))
            return true;
        else
            return false;
    }
